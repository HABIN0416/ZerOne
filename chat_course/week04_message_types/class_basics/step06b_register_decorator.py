...
# ────────────────────────────────────────────
# 1. 비밀 1: 클래스도 '값'이다★★★★★object아님!! 붕어방 틀도 값이다!!!!
# ────────────────────────────────────────────
print("=== 비밀 1: 클래스도 값이다 ===")
print(IPhone)                         # 클래스 '자체'를 출력할 수 있다 (<class ...>) ★파이썬만 class가 value로 가능.★

box = IPhone                          # 변수에 담을 수 있다! (괄호 없음 = 호출 아님) ★그래서 ★
p = box("철수", "010-1111-1111")      # 담은 것으로 객체도 만들 수 있다 ★p부터는 붕어빵! box("철수"~)는 붕어빵 틀★
p.ring()

d = {"IPHONE": IPhone}                # 딕셔너리의 '값'으로도 넣을 수 있다 ★값이니까 딕셔너리로 담을 수도 있음★
p2 = d["IPHONE"]("영희", "010-2222-2222")   # 꺼내서 바로 객체 만들기 ★p2는 붕어빵! d뒤로는 붕어빵 틀! 직관적으로 이해하기★
p2.ring()
...
# ────────────────────────────────────────────
REGISTRY = {}                         # 대리점 카탈로그: 기종코드 → 클래스 ★이거 registry 그냥 변수임 착각 ㄴㄴ★


def register(phone_class):            # 클래스를 받아서★붕어빵틀을 받아서★
    print(f"   [카탈로그 등록] {phone_class.tag}")
    REGISTRY[phone_class.tag] = phone_class   # 카탈로그에 올리고 ★tag에 아이폰이나 구글폰을 받는 것★
    return phone_class                # ★ 받은 그대로 돌려준다 (이유는 4번에서!) ★★★★★★★★★★★★return 붕어빵 틀을 받아서 그 값을 register?로 받는다?2:25:0,,,까지


print("\n=== 보통 함수 호출로 등록해 보기 ===")
register(IPhone)                      # 숫자 넘기듯 클래스를 넘겼다 ★클래스 등록한거=신제품 등록★
print("카탈로그:", REGISTRY) #★카다로그에 폰 등록★
