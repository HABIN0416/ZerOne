...
iphone.call("010-2222-2222")     # 이것도 부모 것 #to 매개변수
iphone.facetime("영희")          # 이건 자기 것 #to 매개변수
iphone.sms("영희", "지금 뭐해?") #부모 클래스인  Phone 에서 불러옴

# gphone = GPhone("영희", "010-2222-2222")
# gphone.power_on()
# gphone.sms("010-1111-1111", "밥 먹었어?") ##to, text 매개변수
# gphone.circle_search("맛집") #thing매개변수
...
class FoldPhone(Phone):
    def __init__(self, owner, number, fold):
        super().__init__(owner, number)
        self.fold = fold

    def fold_on(self): #####fold 이름으로 쓰면 안됨
        self.power_off()
        print(f"{self.owner}의 폰 {self.fold} 실행! 접혀서 꺼졌다?")

    def unfold(self):
        self.power_on()
        print(f"{self.owner}의 폰이 {self.fold} X! 펴져서 켜짐!")

print("\n=== super(): 부모의 준비 + 나의 준비 ===")
sp = SmartPhone("민수", "010-3333-3333", "카카오톡")
sp.power_on()                            # 부모 __init__ 이 만든 power 가 잘 있다
sp.run_app()
fp = FoldPhone("빈", "010-1111-1123", "폴드")
fp.fold_on()
fp.unfold()
